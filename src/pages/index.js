    ...
    <>
      ...
      <div className="container mx-auto">
        <div className="my-6 space-x-5">
          <button
            className={`border-[#d7615b] rounded-full  border-2 py-1 px-3 ${
              !typeFilter && "bg-[#d7615b] text-white"
            } `}
          ...
          <button
            className={`border-[#d7615b] rounded-full  border-2 py-1 px-3 ${
              typeFilter === "Veg" && "bg-[#d7615b] text-white"
            } `}
            onClick={() => {
              setTypeFilter("Veg");
            }}
          >
            <span
              className={
                "lowercase font-thin bg-white border-green-500 border mr-2 w-7 h-7 text-green-500"
              }
            >
              ●
            </span>
          ...
          <button
            className={`border-[#d7615b] rounded-full dark:border-white border-2 py-1 px-3 ${
              typeFilter === "Non-Veg" && "bg-[#d7615b] text-white"
            } `}
            onClick={() => {
...
export async function getStaticProps() {
  let data;
  try {
    const IndianFoodData = await fetch(baseUrl + "api/foodData", { method: "GET" })
      .then((response) => response.json())
      .catch((error) => error.message);

    data = await JSON.parse(JSON.stringify(IndianFoodData)); // step required during deployment in staticProps
  } catch (error) {
